...
@section('content')
	<div x-data="customerModal()" class="relative w-full px-4 py-3">
					...
					<table class="w-full text-left border-collapse">
						<thead>
							<tr class="text-sm border-b text-gray-500">
								<th class="py-2 px-2">Customer Name</th>
						...
						<tbody>
							@foreach ($customer as $cs)
								<tr class="text-sm text-gray-700 border-b hover:bg-gray-50">
									<td class="py-2 px-2">{{ $cs->customer_name }}</td>
									<td class="py-2 px-2 overflow-hidden text-ellipsis whitespace-nowrap">{{ $cs->email }}</td>
									<td class="py-2 px-2">{{ $cs->phone }}</td>
									<td class="py-2 px-2 whitespace-nowrap overflow-ellipsis line-clamp-1">{{ $cs->address }}</td>
									<td class="py-2 px-2"> <span
											class="px-2 py-1 text-xs rounded-lg bg-green-100 text-green-600 {{ $cs->status ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}
										}}">
									...
									<td class="py-2
										...
										<div class="relative ">
											...
											<div x-show="open" @click.outside="open = false" x-transition
												...
												<button @click="openModal({{ $cs->customer_id }}, 'view'); open = false"
													class="block text-left px-3 py-2 text-sm text-gray-700 w-full hover:bg-gray-100 cursor-pointer">
													Detail
												</button>
												<button @click="openModal({{ $cs->customer_id }}, 'edit'); open = false"
													class="block text-left px-3 py-2 text-sm hover:bg-gray-100 w-full">
													Edit
												</button>
												<form action="{{ route('customer.delete', $cs->customer_id) }}" method="POST"
													onsubmit="return confirm('Are you sure to delete this customer?')">
													@csrf
													@method('DELETE')
		...
		function customerModal() {
			return {
				...
				async openModal(id, mode = 'view') {
					...
					try {
						const response = await fetch(`/customer/${id}?mode=${mode}`);
