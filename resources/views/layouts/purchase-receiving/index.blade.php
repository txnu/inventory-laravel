...
@section('content')
			...
			<div class="flex flex-col gap-4">
				...
				<div class="flex justify-between items-center gap-4">
					...
					<div class="flex items-center gap-2">
						...
						<a href="{{ route('purchase-receiving.create') }}"
						...
						<tbody>
							@foreach ($purchase_receivings as $pr)
								<tr class="text-sm text-gray-700 border-b hover:bg-gray-50">
									...
									@php
										$statusColors = [
										    'partial' => 'bg-orange-200 text-orange-600 font-medium',
                                    ...
                                    {{ $statusColors[$pr->status] ?? 'bg-gray-100 text-gray-600' }}">
											{{ ucfirst($pr->status) }}
										</span>
									</td>
									<td class="py-2 px-2 text-center" x-data="{ open: false }">
										<div class="relative ">
											...
											<div x-show="open" @click.outside="open = false" x-transition
												...
												<button @click="openModal({{ $pr->pr_id }}, 'show'); open = false"
													class="block text-left px-3 py-2 text-sm text-gray-700 w-full hover:bg-gray-100 cursor-pointer">
													Detail
												</button>
												<button @click="openModal({{ $pr->pr_id }}, 'edit'); open = false"
													class="block text-left px-3 py-2 text-sm hover:bg-gray-100 w-full">
													Edit
												</button>
												<form action="{{ route('purchase-receiving.delete', $pr->pr_id) }}" method="POST"
	...
	<script>
		function purchaseReceivingModal() {
			return {
				showModal: false,
				loading: false,
				modalContent: '',
				...
				async openModal(id, mode = 'show') {
					...
					let url =
						mode === 'edit' ?
						`/purchase-receiving/edit/${id}` :
						`/purchase-receiving/show/${id}`;
