						...
						<tr>
							<td class="border-t border-b border-gray-300 px-2 py-2">
								<select x-model="item.product_id" :name="`items[${index}][product_id]`"
									class="w-full border border-gray-300 rounded-md px-2 py-1" {{ $locked ? 'disabled' : '' }} disabled>
							...
							<td class="border-t border-b border-gray-300 px-2 py-2">
								<input type="number" min="1" x-model="item.qty_ordered" @input="updateRow(index)"
									:name="`items[${index}][qty_ordered]`" class="w-full border border-gray-300 rounded-md px-2 py-1"
									{{ $locked ? 'disabled' : '' }} readonly>
							</td>

							<td class="border-t border-b border-gray-300 px-2 py-2">
								<input type="number" min="0" x-model="item.price" @input="updateRow(index)"
									:name="`items[${index}][price]`" class="w-full border border-gray-300 rounded-md px-2 py-1"
									{{ $locked ? 'disabled' : '' }} readonly>
							</td>

							<td class="border-t border-b border-gray-300 px-2 py-2">
								<input type="number" x-model="item.total" :name="`items[${index}][total]`"
									class="w-full border border-gray-300 rounded-md px-2 py-1 bg-gray-100" readonly>
							</td>
						</tr>
					</template>
				</tbody>
